...
def function(gt,x,y,z):
    return 100 * (x + 10)*np.sin(5 * y - 20 * gt)
...
class CalcMesh:
    ...
    def __init__(self, nodes_coords, tetrs_points):
        ...
        self.smth = function(0,self.nodes[0],self.nodes[1],self.nodes[2])

        # Тут может быть скорость, но сейчас здесь что-то
    ...
    def move(self, tau, gt):
        # По сути метод просто двигает все точки c их текущими скоростями
        self.nodes += self.velocity * tau
        self.velocity[0] = (self.nodes[2]/2 + 1) * (5 - self.nodes[2])
        self.velocity[2] = (self.nodes[2]/2 + 1) * (self.nodes[0])
        self.smth = function(gt,self.nodes[0],self.nodes[1],self.nodes[2])
    ...
    def snapshot(self, snap_number):
        ...
        writer.SetFileName("ban3d-step-" + str(snap_number) + ".vtu")
        writer.Write()
...
try:
    ...
    gmsh.merge(os.path.join(path, 'ban.stl'))
except:
